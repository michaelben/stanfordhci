...
dir1=$project/snippets-nex1000-michael
mkdir $dir1
...
do
  ...
  do
    f1_base=${f1##*/};
    if inarray $f1_base "discarded.txt" "daily_report.txt" "daily_report.json" "discarded.txt.1" "daily_report.txt.1" "daily_report.json.1";
    then
      continue;
    else
      f2=$dir2/${f1##*/};
  ...
  do
    f2_base=${f2##*/};
    if inarray $f2_base "discarded.txt" "daily_report.txt" "daily_report.json" "discarded.txt.1" "daily_report.txt.1" "daily_report.json.1";
    then
      continue
    else
      if [[ -d $f2 && $f2 != *__ ]]; then
